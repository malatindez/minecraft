...
enum current_state { CS_INTERFACE, CS_CAMERA };
int main() {
  ...
  GLFWwindow* window = glfwCreateWindow(
      SCR_WIDTH, SCR_HEIGHT, "kinda minecraft version i.d.k.", NULL, NULL);
  if (window == NULL) {
  ...
  unsigned int destroy_stage[10] =
      {loader.LoadTexture(
           "resources\\minecraft\\gui\\blocks\\destroy_stage_0.png"),
       loader.LoadTexture(
           "resources\\minecraft\\gui\\blocks\\destroy_stage_1.png"),
       loader.LoadTexture(
           "resources\\minecraft\\gui\\blocks\\destroy_stage_2.png"),
       loader.LoadTexture(
           "resources\\minecraft\\gui\\blocks\\destroy_stage_3.png"),
       loader.LoadTexture(
           "resources\\minecraft\\gui\\blocks\\destroy_stage_4.png"),
       loader.LoadTexture(
           "resources\\minecraft\\gui\\blocks\\destroy_stage_5.png"),
       loader.LoadTexture(
           "resources\\minecraft\\gui\\blocks\\destroy_stage_6.png"),
       loader.LoadTexture(
           "resources\\minecraft\\gui\\blocks\\destroy_stage_7.png"),
       loader.LoadTexture(
           "resources\\minecraft\\gui\\blocks\\destroy_stage_8.png"),
       loader.LoadTexture(
           "resources\\minecraft\\gui\\blocks\\destroy_stage_9.png")},
               destroy_none = loader.LoadTexture(
                   "resources\\minecraft\\gui\\blocks\\destroy_none.png"),
               hover = loader.LoadTexture(
                   "resources\\minecraft\\gui\\blocks\\hover.png");
  ...
  w.updatePlayerCoords(Block::Coords(
...
#include <thread>
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    cubeShader.use();
    glm::mat4 projection =
        glm::perspective(glm::radians(player.cam.Zoom),
    ...
    int32_t prevHoveredBufx = 2147483647, prevHoveredBufy = 2147483647,
            prevHoveredBufz = 2147483647;

    for (double i = 0; i < 5; i += 1) {
      glm::vec3 g = player.cam.Position + glm::vec3(player.cam.Front.x * i,
                                                    player.cam.Front.y * i,
                                                    player.cam.Front.z * i);
      int32_t x = (int32_t)std::round(g.x);
      int32_t y = (int32_t)std::round(g.y);
      int32_t z = (int32_t)std::round(g.z);
      if (x >= 0 && x < (int32_t)(16 * renderDistance) && z >= 0 &&
          z < (int32_t)(16 * renderDistance)) {
        if (w.getBlock(x, y, z) != nullptr) {
          if (i > 2 &&
              (prevHoveredBufx >= 0 &&
               prevHoveredBufx < (int32_t)(16 * renderDistance) &&
               prevHoveredBufz >= 0 &&
               prevHoveredBufz < (int32_t)(16 * renderDistance)) &&
              (placement)) {
            if (lastFrame - placementStart > 0) {
              placementStart = lastFrame + 0.25;
              w.placeBlock(prevHoveredBufx, prevHoveredBufy, prevHoveredBufz,
                           &(blocks[1]));
          ...
          if (breaking) {
            state = true;
            if (prevhoveredx != hoveredx || prevhoveredy != hoveredy ||
                prevhoveredz != hoveredz) {
              prevhoveredx = hoveredx;
              prevhoveredy = hoveredy;
              prevhoveredz = hoveredz;
              breaking = false;
            } else if (int(5.0f * (lastFrame - breakingStart) * 100 /
                           w.getBlock(x, y, z)->ref->hardness) >= 10) {
              breaking = false;
              glActiveTexture(GL_TEXTURE15);
              glBindTexture(GL_TEXTURE_2D, destroy_none);
              w.breakBlock(hoveredx, hoveredy, hoveredz);
              hoveredx = 2147483647, hoveredy = 2147483647,
              hoveredz = 2147483647;
            } else {
              glActiveTexture(GL_TEXTURE15);
              glBindTexture(
                  GL_TEXTURE_2D,
                  destroy_stage[int(5.0f * (lastFrame - breakingStart) * 100 /
                                    w.getBlock(x, y, z)->ref->hardness)]);
            }
          }
          break;
        }
    ...
    glm::mat4 model(1.0f);
    if (hoveredx >= 0 && hoveredx < (int32_t)(16 * renderDistance) &&
        hoveredz >= 0 && hoveredz < (int32_t)(16 * renderDistance)) {
      glActiveTexture(GL_TEXTURE14);
      glBindTexture(GL_TEXTURE_2D, hover);
