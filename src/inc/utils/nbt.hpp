...
typedef std::variant<TagEnd, TagByte, TagShort, TagInt, TagLong, TagFloat,
                     TagDouble, TagByteArray, TagString, TagList, TagCompound,
...
struct TagList {
  TagList() = default;
  template <typename T>
  TagList(const std::vector<T> &base) : base{base} {}
  template <typename T>
  TagList(std::initializer_list<T> lst)
  ...
  std::variant<std::vector<TagEnd>, std::vector<TagByte>, std::vector<TagShort>,
               std::vector<TagInt>, std::vector<TagLong>, std::vector<TagFloat>,
               std::vector<TagDouble>, std::vector<TagByteArray>,
               std::vector<TagString>, std::vector<TagList>,
               std::vector<TagCompound>, std::vector<TagIntArray>,
  ...
  template <typename T>
  TagList &operator=(const std::vector<T> &other) {
    base = other;
    return *this;
  }
...
template <typename T>
const std::vector<T> &get_list(const TagList &list) {
  return std::get<std::vector<T>>(list.base);
}
template <typename T>
std::vector<T> &get_list(TagList &list) {
  return std::get<std::vector<T>>(list.base);
}

struct TagCompound {
  TagCompound() = default;
  TagCompound(std::initializer_list<std::pair<const std::string, nbt::Tag>> lst)
      : base{lst} {}
  ...
  Tag &operator[](const std::string &key) { return base[key]; }
  Tag &operator[](const char *key) { return base[key]; }

  Tag &at(const std::string &key) { return base.at(key); }
  const Tag &at(const std::string &key) const { return base.at(key); }
...
struct NBT : public TagCompound {
  NBT() = default;
  NBT(std::istream &buf) { decode(buf); };
  NBT(std::istream &&buf) { decode(buf); }
  ...
  void decode(std::istream &&buf) { decode(buf); }

  void encode(std::ostream &buf) const;
  void encode(std::ostream &&buf) const { encode(buf); }

  operator bool() const { return !name && base.empty(); }
...
template <std::integral T>
void encode(std::ostream &buf, const T val) {
...
template <std::integral T>
std::vector<T> decode_array(std::istream &buf) {
  std::int32_t len;
...
void print_array(std::ostream &os, const auto &vec) {
  ...
  if (const int size{static_cast<int>(vec.size())}; size) {
    os << +vec[0];
    for (int i{1}; i < (size < 7 ? size : 3); i++) os << ", " << +vec[i];
    if (size > 7) os << ", and " << size - 3 << " more";
  }
  os << "}";
}
...
#define X(enum, type)                              \
  case (enum): {                                   \
    ...
    for (auto &val : vec) val = decode<type>(buf); \
    return vec;                                    \
  };
      ALL_NUMERIC(X)
#undef X
...
#define X(enum, type, base_type)                              \
  case (enum): {                                              \
    std::vector<type> vec(len);                               \
    for (auto &val : vec) val = decode_array<base_type>(buf); \
    return vec;                                               \
  };
      ALL_ARRAYS(X)
#undef X
...
#define X(enum, type, ext)                        \
  case enum: {                                    \
    std::vector<type> vec(len);                   \
    for (auto &val : vec) val = decode##ext(buf); \
    return vec;                                   \
  };
      ALL_OTHERS(X)
#undef X
...
#define X(enum, type)                                     \
  case enum: {                                            \
    ...
    for (const auto val : vec) encode<type>(buf, val);    \
  } break;
      ALL_NUMERIC(X)
#undef X
...
#define X(enum, type, base_type)                                   \
  case enum: {                                                     \
    auto &vec{get_list<type>(list)};                               \
    encode<TagInt>(buf, static_cast<TagInt>(vec.size()));          \
    for (const auto &val : vec) encode_array<base_type>(buf, val); \
  } break;
      ALL_ARRAYS(X)
#undef X
...
#define X(enum, type, ext)                                \
  case enum: {                                            \
    auto &vec{get_list<type>(list)};                      \
    encode<TagInt>(buf, static_cast<TagInt>(vec.size())); \
    for (const auto &val : vec) encode##ext(buf, val);    \
  } break;
      ALL_OTHERS(X)
#undef X
  }
