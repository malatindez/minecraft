...
namespace resource {
template <typename T> class __AtomicSharedPtr {
public:
  ...
  // be terminated.
  constexpr __Locked Lock() const noexcept {
    return __Locked(mutex_, obj_, false);
  }
  constexpr __Locked TryLock() const noexcept {
    return __Locked(mutex_, obj_, true);
  }
