...
class Chunk {
  ...
  *** Except for data, data is written by block in any way it wants ***
  ...
  00 00 00 00  00 00 00 00  00 00 00 00  00 00 00 00   }
  00 00 00 00  00 00 00 00  00 00 00 00  00 00 00 00     } total size is
  block_data_size bytes
  ...                                                    }
  ...                                                  }
  00 00 00 00  00 00 00 00  00 00 00 00  00 00 00 00 }
  ...
  Blocks, !included in this file(by x y z coordinates) are counted as
  minecraft:air(id is 0).
  */
  Block**** ChunkBlocks;
  ...
  Chunk(uint8_t* data, uint32_t data_size, std::vector<UBlock>* blocks,
        int32_t x, int32_t z, std::queue<std::pair<uint32_t*, uint32_t>>*,
        std::queue<std::pair<uint32_t*, uint32_t>>*, std::mutex*);
  Chunk(const Chunk& b);
  Chunk(const Chunk* b);
  bool isOptimized() { return optimized; }
  bool isVerticesLoaded() { return optimized2; }
...
class World {
 private:
  ...
  std::string name;
  const uint64_t seed;
  enum class Exceptions { CHUNK_SIZE_OVER_16MB = 0 };
  PerlinNoise noise;
 ...
 private:
  ...
  World(TextureLoader* tloader, std::vector<UBlock>* blocks, std::string name,
        uint64_t s, uint32_t renderDistance)
      : seed(s), noise(s), playerCoords(0, 0, 0) {
  ...
  void dynamicGenerationThread() {
    while (!stopThreads) {
      ...
      std::pair<int32_t, int32_t> coords = generationQueue.front();
      generationQueue.erase(generationQueue.begin());
      generationQueueMutex.unlock();
      // loadedChunksMutex.lock();
      // if (std::find(loadedChunks.begin(), loadedChunks.end(), coords) ==
      // loadedChunks.end()) {
      ...
      optimizationQueue.push_back(
          std::pair<int32_t, int32_t>(coords.first, coords.second));
  ...
  void dynamicDeletionThread() {
    while (!stopThreads) {
      ...
      /*if (! this->deleteChunk(coords.first, coords.second)) {
              deletionQueueMutex.lock();
              bool flag = false;
              for (auto itr2 = deletionQueue.begin(); itr2 !=
      deletionQueue.end(); itr2++) { if (coords.first == (*itr2).first &&
      coords.second == (*itr2).second) { flag = true; break;
                      }
              }
              if (!flag) {
                      deletionQueue.push_back(std::pair<int32_t,
      int32_t>(coords.first, coords.second));
              }
              deletionQueueMutex.unlock();
      }*/
    }
  ...
  void dynamicOptimizationThread() {
    while (!stopThreads) {
      ...
      std::pair<int32_t, int32_t> coords = optimizationQueue.front();
      optimizationQueue.erase(optimizationQueue.begin());
      optimizationQueueMutex.unlock();
      loadedChunksMutex.lock();
      if (std::find(loadedChunks.begin(), loadedChunks.end(), coords) ==
          loadedChunks.end()) {
  ...
  void threadManipulator() {
    ...
    while (!stopThreads) {
      ...
      for (int32_t i = -renderDistance + current_coords.x / 16;
           i < renderDistance + current_coords.x / 16; i++) {
        for (int32_t j = -renderDistance + current_coords.z / 16;
             j < renderDistance + current_coords.z / 16; j++) {
          bool flag = false;
          for (auto itr = loadedChunks.begin(); itr != loadedChunks.end();
               itr++) {
      ...
      if (localgenerationQueue.size() > 0) {
        generationQueueMutex.lock();
        generationQueue.insert(generationQueue.end(),
                               localgenerationQueue.begin(),
                               localgenerationQueue.end());
        generationQueueMutex.unlock();
      }

      if (localloadedChunks.size() > 0) {
        loadedChunksMutex.lock();
        loadedChunks.insert(loadedChunks.end(), localloadedChunks.begin(),
                            localloadedChunks.end());
        loadedChunksMutex.unlock();
      }
      ...
      if (i % 10 == 0) {
        ...
        for (auto itr = loadedChunks.begin(); itr != loadedChunks.end();
          ...
          if ((*itr).first < -renderDistance + current_coords.x / 16 ||
            ...
            deletionQueueMutex.lock();
            bool flag2 = false;
            for (auto itr2 = deletionQueue.begin(); itr2 != deletionQueue.end();
                 itr2++) {
            ...
            newLoadedChunks.push_back(
                std::pair<int32_t, int32_t>((*itr).first, (*itr).second));
          }
        }
        if (flag) {
          loadedChunksMutex.lock();
      ...
      if (i % 100 == 0) {
        ...
        for (auto itr = this->chunks.begin(); itr != this->chunks.end();
             itr++) {
          if (std::find(loadedChunks.begin(), loadedChunks.end(),
                        std::pair<int32_t, int32_t>((*itr)->x, (*itr)->z)) ==
              loadedChunks.end()) {
            deletionQueueMutex.lock();
            deletionQueue.push_back(
                std::pair<int32_t, int32_t>((*itr)->x, (*itr)->z));
            loadedChunksMutex.lock();
            std::remove(loadedChunks.begin(), loadedChunks.end(),
                        std::pair<int32_t, int32_t>((*itr)->x, (*itr)->z));
            std::remove(checked.begin(), checked.end(),
                        std::pair<int32_t, int32_t>((*itr)->x, (*itr)->z));
            std::remove(doubleChecked.begin(), doubleChecked.end(),
                        std::pair<int32_t, int32_t>((*itr)->x, (*itr)->z));
            loadedChunksMutex.unlock();
            deletionQueueMutex.unlock();
          }
      ...
      if (i % 200 == 0) {
        ...
        for (auto itr = loadedChunks.begin(); itr != loadedChunks.end();
             itr++) {
          if (std::find(checked.begin(), checked.end(), (*itr)) ==
              checked.end()) {
            pairs.push_back((*itr));
            checked.push_back((*itr));
          } else if (std::find(doubleChecked.begin(), doubleChecked.end(),
                               (*itr)) == doubleChecked.end()) {
            pairs.push_back((*itr));
        ...
        if (pairs.size() > 0) {
          optimizationQueueMutex.lock();
          optimizationQueue.insert(optimizationQueue.end(), pairs.begin(),
                                   pairs.end());
          optimizationQueueMutex.unlock();
        }
      }
  ...
  std::thread *manipulator = nullptr, *generation = nullptr,
              *deletion = nullptr, *optimization = nullptr;
 ...
 public:
  ...
  bool optimizeRenderer(int32_t x, int32_t z) {
    ...
    if (returnValue &&
        (chunks[0] == nullptr || chunks[1] == nullptr && chunks[2] == nullptr ||
         chunks[3] == nullptr)) {
      playerCoordsMutex.lock();
      auto current_coords = playerCoords;
      playerCoordsMutex.unlock();
  ...
  void updateVertices() {
    ...
    if (!deleteVAOs.empty()) {
      VAO_VBOmutex.lock();
      auto vao = deleteVAOs.front();
      deleteVAOs.pop();
      auto vbo = deleteVBOs.front();
      deleteVBOs.pop();
      VAO_VBOmutex.unlock();
  ...
  Block* getBlock(int32_t x, int32_t y, int32_t z) {
    ...
    for (std::vector<Chunk*>::iterator itr = chunks.begin();
         itr != chunks.end(); itr++) {
      if (((*itr)->x * 16 < x && x < (*itr)->x * 16 + 16) &&
          ((*itr)->z * 16 < z && z < (*itr)->z * 16 + 16)) {
  ...
  bool breakBlock(int32_t x, int32_t y, int32_t z) {
    ...
    Chunk* cref = nullptr;
    for (std::vector<Chunk*>::iterator itr = chunks.begin(); itr !=
    chunks.end(); itr++) {

            if (((*itr)->x * 16 < x && x < (*itr)->x * 16 + 16) && ((*itr)->z *
    16 < z && z < (*itr)->z * 16 + 16)) { cref = (*itr);
            }

            if (((*itr)->x * 16 < x - 16 && x - 16 < (*itr)->x * 16 + 16) &&
    ((*itr)->z * 16 < z && z < (*itr)->z * 16 + 16)) { cs[0] = (*itr);
            }
            if (((*itr)->x * 16 < x + 16 && x + 16 < (*itr)->x * 16 + 16) &&
    ((*itr)->z * 16 < z && z < (*itr)->z * 16 + 16)) { cs[2] = (*itr);
            }
            if (((*itr)->x * 16 < x && x < (*itr)->x * 16 + 16) && ((*itr)->z *
    16 < z - 16 && z - 16 < (*itr)->z * 16 + 16)) { cs[1] = (*itr);
            }
            if (((*itr)->x * 16 < x && x < (*itr)->x * 16 + 16) && ((*itr)->z *
    16 < z + 16 && z + 16 < (*itr)->z * 16 + 16)) { cs[3] = (*itr);
            }
    }
    if (cref != nullptr) {
            try {
    ...
    for (size_t i = 0; i < 4; i++) {
            if(cs[i] != nullptr)
            optimizationQueue.push(std::pair<int32_t, int32_t>(cs[i]->x,
    cs[i]->z));
    }
  ...
  void generateChunk(int32_t x, int32_t z) {
    ...
    for (uint8_t i = 0; i < 16; i++) {
      for (uint8_t j = 0; j < 16; j++) {
        uint32_t height =
            (uint32_t)(30 + 10.0 * noise.noise((double(x) * 16 + i) / 16, 0,
                                               (double(z) * 16 + j) / 16));
        if (height > 255) {
          height = 255;
        }
    ...
    Chunk* returnValue = new Chunk(gdata, size, blocks, x, z, &deleteVAOs,
                                   &deleteVBOs, &VAO_VBOmutex);
    delete[] gdata;
    chunksMutex.lock();
  ...
  void loadChunk(int32_t x, int32_t z) {
    std::string chunk_name =
        std::to_string(x) + "," + std::to_string(z) + ".chunk";
    ::std::ifstream chunk("saves\\" + name + "\\chunks\\" + chunk_name,
                          std::ifstream::binary | std::ifstream::ate);
    Chunk* returnValue = nullptr;
    if (chunk.is_open()) {
      uint32_t chunk_size = (uint32_t)chunk.tellg();
      chunk.close();
      if (chunk_size > 16777216) {  // chunk size is over 16 MB, that means that
                                    // chunk is probably corrupted
        throw Exceptions::CHUNK_SIZE_OVER_16MB;
      }
      uint8_t* data = new uint8_t[chunk_size];
      chunk.get((char*)data, chunk_size);
      chunk.close();
      returnValue = new Chunk(data, chunk_size, blocks, x, z, &deleteVAOs,
                              &deleteVBOs, &VAO_VBOmutex);
      delete[] data;
    } else {
      generateChunk(x, z);
  ...
  static World NewWorld(TextureLoader* t, std::vector<UBlock>* blocks,
                        std::string name, uint32_t renderDistance) {
  ...
  static World OpenWorld(TextureLoader* t, std::vector<UBlock>* blocks,
                         std::string name, uint32_t renderDistance) {
    uint64_t seed;
    ::std::ifstream manifest("saves\\" + name + "\\MANIFEST",
...
Block::Block(uint8_t* data, size_t size, UBlock* block, Coords coords)
    : coords(coords) {
...
Chunk::Chunk(uint8_t* data, uint32_t data_size, std::vector<UBlock>* blocks,
             int32_t x, int32_t z,
             std::queue<std::pair<uint32_t*, uint32_t>>* vaos,
             std::queue<std::pair<uint32_t*, uint32_t>>* vbos,
             std::mutex* mutex)
    : x(x), z(z) {
  this->blocks = blocks;
  ...
  while (offset + 8 < data_size) {
    ...
    offset += 2;
    ChunkBlocks[x][y][z] =
        new Block(&(data[offset]), block_data_size, &((*blocks)[id]),
                  Block::Coords(this->x * 16 + x, y, this->z * 16 + z));
  }
}
  ...
  for (int32_t i = 0; i < 16; i++) {
    for (int32_t j = 16 * yoffset; j < 16 * (yoffset + 1); j++) {
      for (int32_t k = 0; k < 16; k++) {
        if (ChunkBlocks[i][j][k] != nullptr) {
          // if block nearby is minecraft:air - that means that we should
          // process this block otherwise we just skip it
          bool* statements = new bool[6];  // tiles which we should render
          if (k > 0) {
            statements[0] = (ChunkBlocks[i][j][k - 1] == nullptr);
          } else {
            statements[0] = (nearby[1] == nullptr ||
                             (nearby[1]->ChunkBlocks[i][j][15] == nullptr));
          }
          if (k < 15) {
            statements[1] = (ChunkBlocks[i][j][k + 1] == nullptr);
          } else {
            statements[1] = (nearby[3] == nullptr ||
                             (nearby[3]->ChunkBlocks[i][j][0] == nullptr));
          }
          if (i > 0) {
            statements[2] = (ChunkBlocks[i - 1][j][k] == nullptr);
          } else {
            statements[2] = (nearby[0] == nullptr ||
                             (nearby[0]->ChunkBlocks[15][j][k] == nullptr));
          }
          if (i < 15) {
            statements[3] = (ChunkBlocks[i + 1][j][k] == nullptr);
          } else {
            statements[3] = (nearby[2] == nullptr ||
                             (nearby[2]->ChunkBlocks[0][j][k] == nullptr));
          }
          if (j > 0) {
          ...
          if (statements[0] || statements[1] || statements[2] ||
              statements[3] || statements[4] || statements[5]) {
            bool flag = false;
            for (auto itr = blocks.begin(); itr != blocks.end(); itr++) {
              if ((*itr).first->integer_id ==
                  ChunkBlocks[i][j][k]->ref->integer_id) {
                (*itr).second.push_back(
                    std::pair<Block*, bool*>(ChunkBlocks[i][j][k], statements));
                flag = true;
                break;
              }
            }
            if (!flag) {
              blocks.push_back(
                  std::pair<UBlock*, std::vector<std::pair<Block*, bool*>>>(
                      ChunkBlocks[i][j][k]->ref,
                      std::vector<std::pair<Block*, bool*>>()));
              blocks[blocks.size() - 1].second.push_back(
                  std::pair<Block*, bool*>(ChunkBlocks[i][j][k], statements));
            }
          } else {
            delete[] statements;
...
#define PUSH_VERTICES(x, coords) \
  vertices.push_back(std::pair<uint32_t, Block::Coords>(x, coords))
#define PUSH_ALL_VERTICES(offset, coords) \
  PUSH_VERTICES(6 * offset, coords);      \
  PUSH_VERTICES(6 * offset + 1, coords);  \
  PUSH_VERTICES(6 * offset + 2, coords);  \
  PUSH_VERTICES(6 * offset + 3, coords);  \
  PUSH_VERTICES(6 * offset + 4, coords);  \
  PUSH_VERTICES(6 * offset + 5, coords);
  int m = 0;
  for (auto a = blocks.begin(); a != blocks.end(); a++, m++) {
    std::vector<std::pair<uint32_t, Block::Coords>>
        vertices;  // this->vertices[vertices[i]] -> this->vertices[vertices[i]
                   // + 9] - vertice data
    for (auto blockItr = (*a).second.begin(); blockItr != (*a).second.end();
         blockItr++) {
    ...
    for (auto itr = vertices.begin(); itr != vertices.end(); itr++, i++) {
      x[i * 9] =
          this->vertices[9 * (*itr).first] + (*itr).second.x - this->x * 16;
      x[i * 9 + 1] = this->vertices[9 * (*itr).first + 1] + (*itr).second.y;
      x[i * 9 + 2] =
          this->vertices[9 * (*itr).first + 2] + (*itr).second.z - this->z * 16;
...
void Chunk::loadVerticesToVideoMemory() {
  ...
  for (size_t yoffset = 0; yoffset < 16; yoffset++) {
    for (size_t i = 0; i < ovsize[yoffset]; i++) {
      ...
      glBufferData(GL_ARRAY_BUFFER, OVSizel[yoffset][i] * sizeof(float),
                   optimizedVertices[yoffset][i], GL_STATIC_DRAW);
...
void Chunk::deleteOptimizedRenderer(uint16_t yoffset) {
  ...
  if (optimized2) {
    ...
    this->VAODeleteList->push(
        std::pair<uint32_t*, uint32_t>((VAO[yoffset]), ovsize[yoffset]));
    this->VBODeleteList->push(
        std::pair<uint32_t*, uint32_t>((VBO[yoffset]), ovsize[yoffset]));
...
void Chunk::Save() {}
bool Chunk::PlaceBlock(int32_t x, int32_t y, int32_t z, UBlock* block,
                       Chunk** nearby) {
  x -= this->x * 16;
  z -= this->z * 16;
  ...
  ChunkBlocks[x][y][z] = new Block(
      nullptr, 0, block, Block::Coords(x + this->x * 16, y, z + this->z * 16));
