...
class UBlock {  // universal block
 public:
  ...
  uint8_t hardness;
  UBlock(const uint32_t* diffuseTextures, const uint32_t* specularTextures,
         const std::string str_id, uint8_t hardness) {
...
::std::vector<UBlock> loadBlocks(TextureLoader* loader) {
  ...
  for (const auto& entry : ::std::filesystem::directory_iterator(path)) {
    ...
    if (ends_with(blockPath, (".block"))) {
      ...
      for (uint8_t i = 0; i < 12; i++) {
        getline(block, line);
        if (i < 6) {
          diffuseTextures[i] = loader->LoadTexture(
              ("resources\\minecraft\\blocks\\" + line).c_str());
        } else {
          specularTextures[i - 6] = loader->LoadTexture(
              ("resources\\minecraft\\blocks\\" + line).c_str());
        }
      }
      getline(block, line);
      ...
      std::string name = "";
      for (size_t i = blockPath.size() - 7;
           i != size_t(-1) && blockPath[i] != '\\'; i--) {
        name += blockPath[i];
      }
      std::reverse(name.begin(), name.end());
      returnValue.push_back(UBlock(diffuseTextures, specularTextures,
                                   "minecraft:" + name, hardness));
    }
  }
  ...
  while (blocks_manifest.good()) {
    ...
    size_t i = 0;
    std::string integer = "";
    for (; i < line.size() && line[i] != ' '; integer += line[i++])
      ;  // getting ' ' location and writing id to integer
    for (size_t j = 0; j <= i; j++) {
      line.erase(line.begin());
    }
    lines.push_back(
        std::pair<int32_t, ::std::string>(std::atoi(integer.c_str()), line));
  }
  std::vector<UBlock> buf;
  for (std::vector<std::pair<int32_t, ::std::string>>::iterator itr =
           lines.begin();
       itr != lines.end(); itr++) {
    for (std::vector<UBlock>::iterator i = returnValue.begin();
         i != returnValue.end(); i++) {
