...
class Player {
  ...
  World* world = nullptr;
  Input* input;
  KeySequence *forward, *backward, *left, *right, *up, *down, *breakSeq,
      *placeSeq;
  double* deltaTime;  // pointer to location where is deltatime updating
 public:
  Camera cam;
  Player(GLFWwindow* window, World* world, double* deltaTimeLocation,
         Input* input) {
    this->input = input;
 ...
 public:
  ...
  void update() {
    ...
    double yoffset =
        lastY -
        pos.second;  // reversed since y-coordinates go from bottom to top
    ...
    for (double i = 0; i < 5; i += 0.01) {
      glm::vec3 g = cam.Position + glm::vec3(cam.Front.x * i, cam.Front.y * i,
                                             cam.Front.z * i);
